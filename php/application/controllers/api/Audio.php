...
class Audio extends API_Controller {
    ...
    private function _upExpPoints($user, $date)
    {
        ...
        $this->load->model('Audio_play_seconds_model');
        $this->db->transStart();
        //处理积分
        $data = [
            'user_id' => $user['id'],
        ...
        $update = array('point' => $user['point'] + 10, 'exp'=>$user['exp']+1);
        $this->Users_model->update_by(array('id' => $user['id']), $update);
        //更新为已处理
        $this->Audio_play_seconds_model->update_by(array('user_id' => $user['id'], 'date'=>$date), ['is_up_exp_points'=>1, 'updated_at'=>$datetime]);
    ...
    /**
     * @api {post} /api/audio/finish 音频-播放时长记录
     * @apiVersion 1.0.0
     * @apiName audio_finish
     * @apiGroup api
     *
     * @apiSampleRequest /api/audio/finish
     *
     * @apiParam {Number} user_id 用户唯一ID
     * @apiParam {String} sign 校验签名
     * @apiParam {Number} seconds 音频播放时长（秒）
     *
     * @apiSuccess {Number} status 接口状态 0成功 其他异常
     * @apiSuccess {String} message 接口信息描述
     * @apiSuccess {Object} data 接口数据集
