...
export default class GiftManager extends BaseComponent{
	...
	constructor(props) {
		...
		this.columns = [
			...
			}, 
			{
				...
				width: '12%',
				render: (text, record) => this.renderSelect(text, record, 'exchange_type'),
			}, 
			{
				title: '兑换数量',
				dataIndex: 'amount',
				width: '12%',
				render: (text, record) => this.renderInput(text, record, 'amount'),
			},
			{
				title: '启用',
				dataIndex: 'enable',
	...
	renderImg(text,record,column){
		...
		return <div>
			...
			>
				{img?<img className='img-uploader' src={img} alt=''/>:<div>
					<Icon type={loading ? 'loading' : 'plus'} />
					<div className="ant-upload-text">上传</div>
				</div>}
			</Upload>:<img className='img-uploader' src={Base.getImgUrl(img)} alt=''/>}
		</div>
	}
	...
	onSave(id) {
		const list = this.store.list.slice();
		const itemData = list.find(item=>id === item.id);
		Base.POST({act:'live_gift',op:'save',...itemData},(res)=>{
	...
	@action.bound
	onDelete(id){
		Base.POST({act:'live_gift',op:'save',id,deleted:"1"},()=>remove(this.store.list,item=>id === item.id),this);
	}
	//添加
	@action.bound
	onAdd(){
		...
		this.store.list.unshift({id:0,title:'',editable:true,deleted:'0',enable:'1',sort:0,exchange_type:'',img:''});
	}
	//搜索
	searchStr = ''
	@action.bound
	...
	@action.bound
	requestData(){
		Base.GET({act:'live_gift',op:'index',title:this.searchStr || '',cur_page:this.current || 1,per_page:Global.PAGE_SIZE},(res)=>{
