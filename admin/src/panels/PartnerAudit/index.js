...
import { Table, Input,Popconfirm,Button,Spin,message } from 'antd';
...
export default class PartnerAudit extends BaseComponent{
				...
				render: (text, record) => {
					...
					<div className="editable-row-operations">
						<span>
							...
							<Popconfirm title="确认审核不通过?" okText='确定' cancelText='取消' onConfirm={() => this.onSave(id,0)}>
	...
	renderImg(text,record,column){
		const {image} = record;
		return <div>
			<img className='img-uploader' src={Base.getImgUrl(image)} alt=''/>
		</div>
	}
	//删除
	@action.bound
	onSave(id,status){
		Base.POST(
            { act: "partner_city", op: "save",mod:'admin',id, check_status: status },
            () => remove(this.store.list, item => id === item.id),
            this);
	}
	componentDidMount() {
		Base.GET({act:'partner_city',op:'index',mod:'admin'},(res)=>{
			const {list} = res.data;
