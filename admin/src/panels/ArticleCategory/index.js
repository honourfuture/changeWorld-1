...
import './ArticleCategory.less';
const Search = Input.Search;

export default class ArticleCategory extends BaseComponent{
	store={
		list:[],
		searchStr:''
	}
	...
	onSave(id) {
		const list = this.store.list.slice();
		const itemData = list.find(item=>id === item.id);
		Base.POST({act:'article_class',op:'save',...itemData},(res)=>{
	...
	@action.bound
	onDelete(id){
		Base.POST({act:'article_class',op:'save',id,deleted:"1"},()=>remove(this.store.list,item=>id === item.id),this);
	...
	onAdd(){
		...
		this.store.list.unshift({id:0,sort:'0',name:'',editable:true,deleted:'0',enable:'1'});
	}
	//搜索
	@action.bound
	onSearch(e){
	...
	render(){
		...
		return (
			<Spin ref='spin' wrapperClassName='ArticleCategory' spinning={false}>
				<div className='pb10'>
