...
import './AnchorType.less';
const Search = Input.Search;

export default class AnchorType extends BaseComponent{
	store={
		list:[],
		searchStr:''
	}
	...
	onSave(id) {
		...
		Base.POST({act:'anchor_class',op:'save',...itemData,sort:parseInt(itemData.sort,10)},(res)=>{
	...
	@action.bound
	onDelete(id){
		Base.POST({act:'anchor_class',op:'save',id,deleted:"1"},()=>remove(this.store.list,item=>id === item.id),this);
	}
	//添加
	@action.bound
	onAdd(){
	...
	componentDidMount() {
		Base.GET({act:'anchor_class',op:'index'},(res)=>{
			console.log(res)
