...
export default class ShopNavList extends BaseComponent{
	...
	onSave(id) {
		...
		Base.POST({act:'goods_class',op:'save',...itemData,sort:parseInt(itemData.sort,10)},(res)=>{
	...
	@action.bound
	onDelete(id){
		Base.POST({act:'goods_class',op:'save',id,deleted:"1"},()=>remove(this.store.list,item=>id === item.id),this);
	}
	//添加
	@action.bound
	onAdd(){
