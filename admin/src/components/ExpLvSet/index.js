...
import { Table, Input,Popconfirm,Switch,Button,message,Upload,Icon} from 'antd';
...
export class ExpLvSet extends BaseComponent{
		...
		this.columns = [
			{
				...
				width: '10%',
				render: (text, record) => this.renderInput(text, record, 'grade_name'),
			}, 
			{
				title: '晋级值',
				dataIndex: 'grade_demand',
				width: '10%',
				render: (text, record) => this.renderInput(text, record, 'grade_demand'),
			}, 
			{
				title: '等级图',
				dataIndex: 'grade_logo',
				width: '10%',
				render: (text, record) => this.renderImg(text, record, 'grade_logo'),
			},
			{
				title: '启用',
				dataIndex: 'enable',
	...
	renderImg(text, record, column) {
		...
		return <div>
			...
			>
				{grade_logo?<img className='img-uploader' src={grade_logo} alt=''/>:<div>
					<Icon type={loading ? 'loading' : 'plus'} />
					<div className="ant-upload-text">上传</div>
				</div>}
			</Upload>:<img className='img-uploader' src={`${Global.RES_URL}${grade_logo}`} alt=''/>}
		</div>
	}
	...
	onSave(id) {
		...
		Base.POST({act:'grade',op:'save',mod:'admin',...itemData},(res)=>{
	...
	@action.bound
	onDelete(id){
		Base.POST({act:'grade',op:'save',mod:'admin',id,deleted:"1"},()=>remove(this.store.list,item=>id === item.id),this);
	}
	//添加
	@action.bound
	onAdd(){
		...
		this.store.list.unshift({id:'',grade_name:'',grade_demand:'',grade_logo:'',editable:true,deleted:'0',enable:'1'});
	}
	@action.bound
	requestData(){
		Base.POST({act:'grade',op:'index',mod:'admin',cur_page:this.current || 1,per_page:Global.PAGE_SIZE},(res)=>{
	...
	render(){
		...
		return (
			<div className='ExpLvSet'>
				...
				<Table className="mt16" bordered onChange={this.onTableHandler} dataSource={showList} rowKey='id' columns={this.columns} pagination={false} />
			</div>
		)
	}
};
