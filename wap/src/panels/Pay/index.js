...
export default class Pay extends BaseComponent {
    ...
    @action.bound
    changeHandler(index, payType) {
        this.store.curIndex = index;
        this.store.payType = payType;
    }
    @action.bound
    payHandler() {
        ...
        if (parseInt(isPretty) === 1) {
            Base.GET({ act: "pretty", op: "view", mod: "user", id }, res => {
                this.store.data = {
                    ...
                    price_point: res.data.price_point
                };
            });
        } else {
            Base.GET(
    ...
    render() {
        ...
        const { real_total_amount, balance, point, price_point } = data;
            ...
            if(type === 'point' && isPretty){
                return (
                    <Flex
                        justify="between"
                        key={type}
                        onClick={() => this.changeHandler(index, 'point')}
                    >
                        <Flex.Item>
                            <span className="pay-gold">积分支付</span>
                            <span className="account-money">
                                积分余额：{Base.getNumFormat(point, 0)}
                            </span>
                        </Flex.Item>
                        <Flex.Item>
                ...
                return (
                    <Flex
                        justify="between"
                        key={type}
                        onClick={() => this.changeHandler(index, 'balance')}
                    >
                        <Flex.Item>
                ...
                return (
                    <Flex
                        justify="between"
                        key={type}
                        onClick={() => this.changeHandler(index, type)}
                    >
                        <Flex.Item>
                    ...
                    <div className="Pay-box">
                        <div className="Pay-box-mode">
                            ...
                            <div className="pay-check">
                                ...
                                <em>{this.store.payType == 'point' ? '抵扣积分：' : '支付金额:'}</em>
                                {this.store.payType != 'point' ?  ('￥' + Base.getNumFormat(real_total_amount) ) : Base.getNumFormat(price_point, 0)}
                                </span>
                            </div>
                        </div>
                        <Flex
